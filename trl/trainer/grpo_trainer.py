    ...
    def __init__(
        ...
        if args.loss_type == "up" and args.importance_sampling_level == "sequence":
            logger.warning(
                ...
                "instead of averaging per-sequence losses as in the UP paper's UP-GSPO setup. To reproduce the UP "
                "paper's setup, set `importance_sampling_level='token'` (the default) for token-level UP-DAPO."
            )
